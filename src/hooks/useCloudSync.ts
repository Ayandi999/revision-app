 ...
 * If a 401 Unauthorized error occurs, automatically forces a silent token refresh
...
export function useCloudSync() {
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        console.warn("[useCloudSync] Init silent sign-in error:", err);
      } finally {
  ...
  const signIn = useCallback(async () => {
    ...
    try {
      ...
      if (!result.success) {
        if (result.cancelled) {
          // User dismissed account picker without choosing
          return false;
        }
      ...
      console.error("[useCloudSync] Sign-in failed:", err);
  ...
  const signOut = useCallback(async () => {
    try {
      ...
      console.error("[useCloudSync] Sign-out failed:", err);
    } finally {
      // Always reset local UI state regardless of remote/storage outcome
      setUser(null);
    }
  }, []);
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              console.error("[useCloudSync] Restore failed:", err);
              Alert.alert(
                "Restore Failed",
